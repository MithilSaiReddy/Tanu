 ...
 *
 * Displays an animated character (pre-decoded RGB565 frames via lv_animimg),
 * status bar, and response ticker directly to a Linux framebuffer (/dev/fb0)
 * via LVGL's fbdev driver. Connects to the Tanu aiohttp WebSocket server to
 * receive state updates and response tokens.
 *
 * Character frames: generated from src/tanu/assets/idle.gif by build_panel.sh
 *   (LVGLImage.py --cf RGB565) into generated/tanu_face_*.c
...
/* Frame descriptors + declarations auto-generated by build_panel.sh */
#include "tanu_face_frames.h"
...
#define ANIM_TICK_MS     50  /* 20 fps — used for the WS/label poll timer */
...
static void create_ui(void) {
    ...
    /* Character animation — below status bar, frames pre-decoded to RGB565 */
    printf("[UI] step1: lv_animimg_create...\n"); fflush(stdout);
    face = lv_animimg_create(scr);
    printf("[UI] step1 result: %p\n", (void*)face); fflush(stdout);
    if (face) {
        printf("[UI] step2: setting src (R5G6B5 frames)...\n"); fflush(stdout);
        lv_animimg_set_src(face, (const void **)tanu_face_frames,
                           TANU_FACE_FRAMES);
        lv_animimg_set_duration(face, TANU_FACE_FRAMES * ANIM_TICK_MS);
        lv_animimg_set_repeat_count(face, LV_ANIM_REPEAT_INFINITE);
        lv_animimg_start(face);
        printf("[UI] step2 done\n"); fflush(stdout);

        lv_obj_set_size(face, scr_w - pad * 2, 224);
        lv_obj_align(face, LV_ALIGN_TOP_MID, 0, STATUS_BAR_H + pad * 2);
        printf("[UI] step3 aligned\n"); fflush(stdout);
    } else {
        printf("[UI] lv_animimg_create returned NULL!\n"); fflush(stdout);
    }
